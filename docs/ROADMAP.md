...
- [ ] 進階報表與數據分析功能（iOS 完成；Android 為可用初版，持續補強）
- [ ] 用戶訂閱狀態與權限管理
- [ ] Pro 功能用戶體驗優化與市場策略
...
- 進階報表與數據分析：iOS `HomeView` 新增 `AdvancedReportViewModel` 與 1/3/6/12 月趨勢摘要、MoM 分類變化分析（Top growth/decline）；Android `HomeScreen` 已改為讀取 `ExpenseRepository` 資料，透過 `AdvancedReportCalculator` 依 `createdAt` 進行區間過濾與平均值摘要，並沿用 paywall 觸發（Free 限 1M）。
