...
- IAP 整合補強：Android 新增 `GooglePlayBillingPurchaseService`（plan/productId 映射、restore tier 判定）與 `GooglePlayBillingGateway` 介面，將後續 BillingClient 實作與 entitlement/domain 決策解耦；新增 `GooglePlayBillingPurchaseServiceTest` 驗證月費映射、未知 SKU 失敗、restore 最高 tier 選取與空購買紀錄情境。
