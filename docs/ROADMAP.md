...
- 進階報表與數據分析：iOS `HomeView` 新增 `AdvancedReportViewModel` 與 1/3/6/12 月趨勢摘要、MoM 分類變化分析（Top growth/decline），Free 僅限 1M，3M+ 觸發 paywall；Android 新增 `AdvancedReportCalculator` + `HomeScreen` 區間切換/平均值摘要，並沿用 paywall 觸發。
- 測試：iOS `BudgetViewModelTests` 新增進階報表權限與 MoM 分析案例；Android 新增 `AdvancedReportCalculatorTest` 驗證 Free/Pro 區間計算行為。
